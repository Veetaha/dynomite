...
use syn::{
    parse::Parse, punctuated::Punctuated, Attribute, DataStruct, DeriveInput, Expr, Field, Fields,
...
impl<'a> ItemField<'a> {
    ...
    fn skip_serializing_if(&self) -> Option<&Expr> {
        self.attrs.iter().find_map(|attr| match &attr.kind {
            FieldAttrKind::SkipSerializingIf(expr) => Some(expr.as_ref()),
            _ => None,
        })
    }
